...
# Association with GOLD3-4 COPD individuals (severe/very severe COPD only)
print("")
print("Starting association for GOLD3-4 COPD subset")
pheno <- pheno[,-c("ratio.irnt","fev1pp.irnt","pef.irnt","PC1","PC2","PC3"),with=F]
pheno <- pheno[GOLDlevel %in% c(3,4)]

print("Loading PCA")
pca <- fread("data/intermediate_files/pca/20-ukbb_gold34_flashpca2_eigenvectors.txt", header=F, stringsAsFactors = F)
pca <- pca[,c("V1","V2","V3","V4"),with=F]
pca[,V1 := gsub("(.*):(.*)","\\2",V1)]
colnames(pca) <- c("eid","PC1","PC2","PC3")
...
for(j in 1:length(phenocols)) {
  ...
  outname <- paste0(outdir,phenoname,".assoc_gold34_v3.csv")
  print(paste0("Saving results to: ", outname))
  fwrite(phenoj_assoc, outname, quote=F, row.names=F, col.names=T)
}
outname <- paste0(outdir,"spirometry_assoc_ukbb_gold34_26a9_v3.csv")
print(paste0("Saving all results of COPD association to: ", outname))
fwrite(results, outname, quote=F, row.names=F, col.names=T)
...
# Association with GOLD2 COPD individuals (moderate COPD only)
print("")
print("Starting association for GOLD2 COPD subset")
pheno <- pheno[,-c("ratio.irnt","fev1pp.irnt","pef.irnt","PC1","PC2","PC3"),with=F]
pheno <- pheno[GOLDlevel %in% 2]

print("Loading PCA")
pca <- fread("data/intermediate_files/pca/22-ukbb_gold2_flashpca2_eigenvectors.txt", header=F, stringsAsFactors = F)
pca <- pca[,c("V1","V2","V3","V4"),with=F]
pca[,V1 := gsub("(.*):(.*)","\\2",V1)]
colnames(pca) <- c("eid","PC1","PC2","PC3")
...
for(j in 1:length(phenocols)) {
  ...
  outname <- paste0(outdir,phenoname,".assoc_gold2_v3.csv")
  print(paste0("Saving results to: ", outname))
  fwrite(phenoj_assoc, outname, quote=F, row.names=F, col.names=T)
}
outname <- paste0(outdir,"spirometry_assoc_ukbb_gold2_26a9_v3.csv")
print(paste0("Saving all results of COPD association to: ", outname))
fwrite(results, outname, quote=F, row.names=F, col.names=T)
